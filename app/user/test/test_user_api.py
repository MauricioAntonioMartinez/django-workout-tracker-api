...
class PublicUserApiTests(TestCase):
    ...
    def test_user_exists(self):
        ...
        payload = {'email': 'test@londonappdev.com',
                   'password': 'testpass', 'name': 'somename'}
        create_user(**payload)
        res = self.client.post(CREATE_USER_URL, payload)
    ...
    def test_create_token_missing_fields(self):
        ...
        """
        res = self.client.post(TOKEN_URL, {'email': 'sd', 'password': ''})
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertNotIn('token', res.data)
...
class PrivateUserApiTest(TestCase):
    ...
    def setUp(self):
        self.user = create_user(email='test@test.com',
                                password='123123', name='Mauricio')
        self.client = APIClient()
        self.client.force_authenticate(user=self.user)

    def test_retrive_profile_success(self):
        """Test retriving profile logged in used
        """
